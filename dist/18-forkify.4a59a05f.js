...
const uploadRecipe = async function(newRecipe) {
    try {
        const ingredients = Object.entries(newRecipe).filter((entry)=>entry[0].startsWith('ingredient') && entry[1] !== '').map((ing)=>{
            ...
            if (ingArr.length !== 3) throw new Error('Wrong ingredient format! Use: Quantity,Unit,Description');
...
const KEY = '31b76914-9e27-4f67-8752-ef740a6a7762';
...
class ResultsView extends (0, _viewJsDefault.default) {
    ...
    _message = '';
    _generateMarkup() {
        return this.data.map((result)=>(0, _previewViewJsDefault.default).render(result, false)).join('');
    }
}
...
class BookmarksView extends (0, _viewJsDefault.default) {
    ...
    _generateMarkup() {
        return this.data.map((bookmark)=>(0, _previewViewJsDefault.default).render(bookmark, false)).join('');
    }
}
